...
Here's some out put from that, but check the full notebook for better context
{{< embed test_learner.ipynb >}}

This is some scratch code.
<details>
```python
class Learner():
  def __init__(self, model, loss_func, optimizer, data):
    self.model = model
...
data = DataLoader([(x,y) for x,y in zip(xs,ys)], batch_size=1, shuffle=True)
